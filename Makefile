...
GENERIC_COMPILER_FLAGS := ${COMPILER_FLAGS}
GENERIC_LINKER_FLAGS := ${LINKER_FLAGS}
...
ifeq ($(CLANG_GCC),)
    ...
    GENERIC_COMPILER_FLAGS := ${GENERIC_COMPILER_FLAGS} --gcc-install-dir=/usr/lib/gcc/x86_64-linux-gnu/${CLANG_GCC}
endif
...
ifeq ($(BUILD),Release)
    ...
    GENERIC_COMPILER_FLAGS := ${GENERIC_COMPILER_FLAGS} -fno-omit-frame-pointer
    GENERIC_LINKER_FLAGS := -lprofiler -ltcmalloc
    DEBUG := gdb
...
ifeq ($(DEBUG),)
    ...
    GENERIC_COMPILER_FLAGS := ${GENERIC_COMPILER_FLAGS} -fsanitize=address -fno-omit-frame-pointer -fno-optimize-sibling-calls
    GENERIC_LINKER_FLAGS := ${GENERIC_LINKER_FLAGS} -fsanitize=address
else
...
CMAKE_ENGINE_COMPILER_FLAGS := \
    -D'CMAKE_C_FLAGS'='${NATIVE_C_COMPILER_FLAGS} ${ARCH_FLAGS} ${GENERIC_COMPILER_FLAGS}' \
    -D'CMAKE_CXX_FLAGS'='${NATIVE_CXX_COMPILER_FLAGS} ${ARCH_FLAGS} ${GENERIC_COMPILER_FLAGS}'

# FIXME: Maybe we have to split engine and game linker flags.
CMAKE_LINKER_FLAGS := -D'CMAKE_EXE_LINKER_FLAGS'='${GENERIC_LINKER_FLAGS}'
...
ifeq ($(VM),dll)
    ...
    CMAKE_GAME_COMPILER_FLAGS := \
        -D'CMAKE_C_FLAGS'='${ARCH_FLAGS} ${GENERIC_COMPILER_FLAGS}' \
        -D'CMAKE_CXX_FLAGS'='${ARCH_FLAGS} ${GENERIC_COMPILER_FLAGS}'
endif
